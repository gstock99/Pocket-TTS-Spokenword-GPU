            ...
            if is_cuda:
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        # ~1.5 GB per worker model copy; leave 2 GB headroom
                        vram_based_workers = max(1, int((vram_gb - 2.0) / 1.5))
                        logger.info(f"GPU mode: {vram_gb:.1f} GB VRAM → up to {vram_based_workers} workers")
                        cpu_based_workers = vram_based_workers
                    else:
