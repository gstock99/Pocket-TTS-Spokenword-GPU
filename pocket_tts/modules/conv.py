...
class StreamingConv1d(StatefulModule):
    ...
    def init_state(self, batch_size: int, sequence_length: int) -> dict[str, torch.Tensor]:
        ...
        previous = torch.zeros(batch_size, self.conv.in_channels, kernel - stride, device=device, dtype=dtype)
...
class StreamingConvTranspose1d(StatefulModule):
    ...
    def init_state(self, batch_size: int, sequence_length: int) -> dict[str, torch.Tensor]:
        ...
        return dict(partial=torch.zeros(batch_size, self.convtr.out_channels, K - S, device=device, dtype=dtype))
